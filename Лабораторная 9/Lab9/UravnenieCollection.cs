...
public class UravnenieCollection
{
    private Uravnenie[] equations;
    private Random random = new Random();

    public int Count => equations.Length;

    public UravnenieCollection()
    {
        equations = new Uravnenie[5];
        for (int i = 0; i < 5; i++)
        {
            equations[i] = new Uravnenie();
        }
    }

    public UravnenieCollection(int size)
    {
        if (size <= 0)
            throw new ArgumentException("Размер массива должен быть больше 0");

        equations = new Uravnenie[size];

        for (int i = 0; i < size; i++)
        {
            double a = random.Next(1, 11);
            double b = random.Next(-10, 11);
            double c = random.Next(-10, 11);

            equations[i] = new Uravnenie(a, b, c);
        }
    }

    public UravnenieCollection(Uravnenie[] equations)
    {
        if (equations == null)
            throw new ArgumentNullException(nameof(equations), "Массив не может быть null");

        this.equations = new Uravnenie[equations.Length];
        Array.Copy(equations, this.equations, equations.Length);
    }

    public void PrintAll()
    {
        Console.WriteLine($"\nКоллекция содержит {equations.Length} уравнений:");
    ...
    private void PrintEquation(Uravnenie eq)
    {
        ...
        get
        {
            if (index < 0 || index >= equations.Length)
                throw new ArgumentOutOfRangeException(nameof(index),
                    $"Индекс {index} выходит за пределы массива [0, {equations.Length - 1}]");

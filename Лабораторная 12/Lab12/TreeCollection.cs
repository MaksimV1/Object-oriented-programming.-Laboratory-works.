...
namespace TreeCollection
{
        ...
        where T : ICloneable, IComparable<T>
    {
        ...
        {
            if (node == null) return null;
            TreeNode<T> newNode = node;//new TreeNode<T>(node.Data);
        ...
        private void InsertToSearchTree(ref TreeNode<T> node, T data)
        {
            ...
            {
                InsertToSearchTree(ref node.Left, data);
            }
            else
            {
                InsertToSearchTree(ref node.Right, data);
            }
        }
        ...
        private TreeNode<T> RemoveNode(TreeNode<T> node, T value, ref bool deleted)
        {
            ...
            if (cmp < 0)
            {
                node.Left = RemoveNode(node.Left, value, ref deleted);
            }
            else if (cmp > 0)
            {
                node.Right = RemoveNode(node.Right, value, ref deleted);
            }
            else
            {
                deleted = true;
        ...
        public double AveragePopulation()
        {
            return Average(item =>
            {
                if (item is Place p) return p.Population;
                return 0.0;
            });
        }
        ...
        public void TransformToSearchTree()
        {
            ...
            {
                InsertToSearchTree(ref root, item);
        ...
        {
            MyCollection<T> clone = new MyCollection<T>(capacity);
            clone.root = CloneNodeDeep(root);
            clone.count = count;
            return clone;
        }
        ...
        public MyCollection<T> ShallowCopy()
        {
            ...
            copy.root = CloneNodeShallow(root);
            copy.count = count;
            return copy;
        }
        ...
        public void ShowInForm()
        {
            if (root == null)
            {
                Console.WriteLine("Дерево пустое, визуализировать нечего.");
                return;
            }

            using (TreeVisualizerForm<T> form = new TreeVisualizerForm<T>(root))
            {
        ...
        {
            bool deleted = false;
            root = RemoveNode(root, item, ref deleted);
            if (deleted)
                count = CountNodes(root);
            return deleted;
        }
